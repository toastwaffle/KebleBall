...
EISITIRIO_DIR = os.path.realpath(__file__).replace("command.py", "eisitirio")
...
def get_app(config):
    ...
    if not config_files:
        ...
        print (
            "Could not load config file {0}".format(
                os.path.join(EISITIRIO_DIR, "config", config_files[0])
            )
        )
    else:
...
MANAGER.add_option(
    "config", default=None, help="Configuration file to load before running commands"
)

MANAGER.add_command("bpython", run_bpython.BpythonCommand)
MANAGER.add_command("cron", cron.CronCommand)
MANAGER.add_command(
    "fix_graduand_postage", fix_graduand_postage.FixGraduandPostageCommand
)
