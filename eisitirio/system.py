...
def check_for_maintenance():
    ...
    if os.path.exists(APP.config["MAINTENANCE_FILE_PATH"]):
        ...
        if (
            "maintenance" not in flask.request.path
...
def static(filename):
    ...
    if os.path.exists(os.path.join(custom_static, filename)):
        return flask.send_from_directory(
            custom_static, filename, cache_timeout=cache_timeout
        )
    else:
        return flask.send_from_directory(
            os.path.join(APP.root_path, "static"), filename, cache_timeout=cache_timeout
        )
...
def error_404(_):
    ...
    LOG(
        ...
        "404 not found for URL {0}, referrer {1}".format(
            flask.request.url, flask.request.referrer
        ),
    )
