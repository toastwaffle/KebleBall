...
def file_lock(lock_file):
    ...
    if os.path.exists(lock_file):
        print (
            ("Only one script can run at once. " "Script is locked with {}").format(
                lock_file
            )
        )
        sys.exit(-1)
...
def send_announcements():
    ...
    announcements = (
        models.Announcement.query.filter(
            models.Announcement.send_email
            == True  # pylint: disable=singleton-comparison
        )
        .filter(
            models.Announcement.email_sent
...
def allocate_waiting():
    ...
    for wait in models.Waiting.query.order_by(models.Waiting.waiting_since).all():
...
def cancel_expired_tickets(now):
    ...
    expired = (
        models.Ticket.query.filter(models.Ticket.expires != None)
        .filter(models.Ticket.expires < now)
        .filter(
...
def remove_expired_secret_keys(now):
    ...
    expired = (
        models.User.query.filter(models.User.secret_key_expiry != None)
        .filter(models.User.secret_key_expiry < now)
        .all()
    )
...
def delete_old_statistics(now):
    ...
    models.Statistic.query.filter(models.Statistic.timestamp < statistic_limit).delete()
...
def generate_statistics():
    ...
    for group in static.STATISTIC_GROUPS:
        DB.session.add_all(
            models.Statistic(group, name, value)
...
class CronCommand(script.Command):
    ...
    def run():
        ...
        lockfile = os.path.abspath("./{}.cron.lock".format(APP.config["ENVIRONMENT"]))
