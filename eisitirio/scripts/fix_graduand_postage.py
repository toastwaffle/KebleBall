...
class FixGraduandPostageCommand(script.Command):
    ...
    def run():
        ...
        with app.APP.app_context():
            graduand_query = models.User.query.join(models.User.affiliation).filter(
...
def needs_fixing(user):
    ...
    return any(
        ticket.postage is None
        or (ticket.postage.postage_type == APP.config["GRADUAND_POSTAGE_OPTION"].name)
        for ticket in user.tickets
    )
...
def get_graduand_postage_entry(user):
    ...
    entry = models.Postage(user, APP.config["GRADUAND_POSTAGE_OPTION"], [], None)
