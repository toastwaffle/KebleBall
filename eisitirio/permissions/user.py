...
def tickets(user):
    ...
    return len([x for x in user.tickets if not x.cancelled]) > 0


@models.User.possession()
def uncollected_tickets(user):
    """Does the user have any uncollected tickets?"""
    return any(not x.cancelled and not x.collected for x in user.tickets) or (
        user.held_ticket is not None and not user.held_ticket.collected
    )
...
def unpaid_tickets(user, method=None):
    ...
    """
    if method is None:
        return any(ticket.can_be_paid_for() for ticket in user.tickets)
    else:
        return any(
...
def paid_tickets(user, method=None):
    ...
    """
    if method is None:
        return len([x for x in user.tickets if (x.paid and not x.cancelled)]) > 0
    else:
        return (
            len(
                [
                    ...
                    if (x.payment_method == method and x.paid and not x.cancelled)
                ]
            )
            > 0
        )
...
def pay_by_battels(user):
    ...
    """
    return user.battels is not None and app.APP.config["CURRENT_TERM"] != "TT"
...
def update_details(user):
    ...
    return user.is_admin or (
        not user.has_held_ticket()
        and not app.APP.config["LOCKDOWN_MODE"]
        and app.APP.config["ENABLE_CHANGING_DETAILS"]
    )
...
def update_photo(user):
    ...
    ):  # verified can be None, pylint: disable=singleton-comparison
        return True
    else:
        return user.is_admin or (
            not user.has_held_ticket()
            and not app.APP.config["LOCKDOWN_MODE"]
            and app.APP.config["ENABLE_CHANGING_PHOTOS"]
        )
