...
def get_postage_query(postage_type, unposted_only=True):
    ...
    postage_query = (
        models.Postage.query.filter(
            models.Postage.paid == True  # pylint: disable=singleton-comparison
        )
        .join(models.Postage.owner)
        .filter(
            models.Postage.cancelled == False  # pylint: disable=singleton-comparison
        )
        .order_by(models.Postage.postage_type)
        .order_by(models.User.surname)
        .order_by(models.User.forenames)
    )
...
def mark_as_posted(postage_id):
    ...
    if not postage:
        ...
        flask.flash(
            "Not all of the tickets for this entry have been assigned.", "error"
        )
    else:
        postage.posted = True
    ...
    return flask.redirect(
        flask.request.referrer or flask.url_for("admin_postage.postage_dashboard")
    )
