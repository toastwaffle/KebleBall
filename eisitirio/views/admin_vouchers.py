...
@ADMIN_VOUCHERS.route("/admin/vouchers/page/<int:page>", methods=["GET", "POST"])
...
def vouchers(page=1):
    ...
    if flask.request.method == "POST":
        ...
        if "voucher_type" not in form or form["voucher_type"] == "":
            ...
            value = util.parse_pounds_pence(
                flask.request.form, "fixed_price_pounds", "fixed_price_pence"
            )
        elif form["voucher_type"] == "Fixed Discount":
            value = util.parse_pounds_pence(
                flask.request.form, "fixed_discount_pounds", "fixed_discount_pence"
            )
            ...
            if value == 0:
                ...
                flask.flash("Cannot give greater than 100% discount", "warning")
                success = False
    ...
    if "search" in flask.request.args:
        voucher_query = voucher_query.filter(
            models.Voucher.code.like("%{0}%".format(flask.request.args["search"]))
        )

    voucher_results = voucher_query.paginate(page, 10)

    return flask.render_template(
        "admin_vouchers/vouchers.html", form=form, vouchers=voucher_results
    )
...
def delete_voucher(voucher_id):
    ...
    if voucher:
        ...
        DB.session.commit()
        flask.flash("Voucher deleted successfully", "success")
    else:
        flask.flash("Could not find voucher to delete", "warning")

    return flask.redirect(
        flask.request.referrer or flask.url_for("admin_vouchers.vouchers")
    )
