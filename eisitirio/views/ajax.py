...
def validate_resale_email():
    ...
    (_, response, _) = validators.validate_resale_email(
        flask.request.form["email"], login.current_user
    )
...
def collect_ticket(ticket_id):
    ...
    if not ticket:
        response = {"success": False, "message": "Could not load ticket."}
    else:
        error = collection_logic.collect_ticket(ticket, flask.request.form["barcode"])
...
def update_waiting(entry_id):
    ...
    if not entry:
        response = {"success": False, "message": "Error: could not load entry."}
    else:
...
def send_verify_code():
    ...
    return flask.Response(
        json.dumps(sms_logic.maybe_send_verification_code(login.current_user)),
        mimetype="text/json",
    )
...
def verify_phone():
    ...
    if flask.request.form["code"] == login.current_user.phone_verification_code:
        ...
        response = {"success": False, "message": "Code mismatch."}
