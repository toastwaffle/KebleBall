...
def do_login():
    ...
    if not user or not user.check_password(flask.request.form["password"]):
        if user:
            APP.log_manager.log_event(
                "Failed login attempt - invalid password", user=user
            )
        else:
            APP.log_manager.log_event(
        ...
        flask.flash("Could not complete log in. Invalid email or password.", "error")
        return flask.redirect(flask.url_for("front.home"))

    if not user.verified:
        APP.log_manager.log_event("Failed login attempt - not verified", user=user)
        flask.flash(
            "Could not complete log in. Email address is not confirmed.", "warning"
        )
        return flask.redirect(flask.url_for("front.home"))
    ...
    login.login_user(
        user,
        remember=(
            "remember-me" in flask.request.form
            and flask.request.form["remember-me"] == "yes"
        ),
    )

    APP.log_manager.log_event("Logged in", user=user)

    flask.flash("Logged in successfully.", "success")
    return flask.redirect(
        flask.request.form.get("next", False)
...
def register():
    ...
    if (
        "password" not in flask.request.form
        or "confirm" not in flask.request.form
        or flask.request.form["password"] != flask.request.form["confirm"]
    ):
    ...
    if "password" not in flask.request.form or flask.request.form["password"] == "":
...
def confirm_email(user_id, secret_key):
    ...
    if user is not None and user.secret_key == secret_key:
        ...
        APP.log_manager.log_event("Confirmed email", user=user)

        if login.current_user.is_anonymous:
            flask.flash(
                "Your email address has been verified. You can now log in", "info"
            )
        else:
            flask.flash("Your email address has been verified.", "info")
    else:  ## XXX/FIXME
        APP.log_manager.log_event("User is not none, or secret key is bad")
    #    flask.flash(
    #        (
...
def email_confirm():
    ...
    if flask.request.method == "POST":
        ...
        if not user:
            APP.log_manager.log_event(
                "Attempted email confirm for {0}".format(flask.request.form["email"])
            )
...
def password_reset():
    ...
    if flask.request.method == "POST":
        ...
        if not user:
            APP.log_manager.log_event(
                "Attempted password reset for {0}".format(flask.request.form["email"])
            )
            ...
            user.secret_key = util.generate_key(64)
            user.secret_key_expiry = datetime.datetime.utcnow() + datetime.timedelta(
                minutes=30
            )
...
def reset_password(user_id, secret_key):
    ...
    if user is None or user.secret_key != secret_key:
        ...
        flask.flash("Could not complete password reset. Please try again", "error")
    ...
    if flask.request.method == "POST":
        ...
        return flask.render_template(
            "front/reset_password.html", user_id=user_id, secret_key=secret_key
        )
    ...
    if user is not None and user.secret_key == secret_key:
        if not user.is_verified:
            for entry in user.events:
                entry.action = (
                    entry.action
                    + " (destroyed user with email address {0})".format(user.email)
                )
                entry.user = None
            ...
            APP.log_manager.log_event(
                "Attempted deletion of verified account", user=user
            )
...
def logout():
    ...
    if "actor_id" in flask.session:
        APP.log_manager.log_event(
            "Finished impersonating user", user=login.current_user
        )
        ...
        if actor:
            login.login_user(actor)

            return flask.redirect(flask.url_for("admin.admin_home"))

    APP.log_manager.log_event("Logged Out", user=login.current_user)
