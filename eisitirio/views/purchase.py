...
def purchase_home():
    ...
    if models.Waiting.query.count() > 0 or not ticket_info.ticket_types:
        flask.flash("You are not able to purchase tickets at this time.", "info")
    ...
    if flask.request.method == "POST":
        ...
        voucher = None
        if (
            "voucher_code" in flask.request.form
        ...
        if flashes:
            flask.flash(
                ("There were errors in your order. Please fix " "these and try again"),
                "error",
            )
            for msg in flashes:
        ...
        if roundup_price is not 0:
            ...
            roundup_donation = models.RoundupDonation(roundup_price, login.current_user)
        ...
        APP.log_manager.log_event(
            "Purchased Tickets", tickets=tickets, user=login.current_user
        )
        ...
        return payment_logic.do_payment(
            tickets, postage, payment_method, payment_term, address
        )
    else:
        return flask.render_template(
            "purchase/purchase_home.html",
...
def upgrade_ticket():
    ...
    price_per_ticket, number_upgrade = purchase_logic.get_ticket_info_for_upgrade(
        login.current_user
    )

    if number_upgrade <= 0 or not ticket_logic.can_buy_upgrade(login.current_user):
        flask.flash("You are not able to upgrade tickets at this time.", "info")
        return flask.redirect(flask.url_for("dashboard.dashboard_home"))

    if flask.request.method == "POST":
        selected_tickets = flask.request.form.getlist("tickets[]")
        if not selected_tickets:
            flask.flash("Please select the tickets you want to upgrade.", "info")
            return flask.redirect(flask.url_for("purchase.upgrade_ticket"))
...
def wait():
    ...
    if flask.request.method != "POST":
        return flask.render_template(
            "purchase/wait.html", wait_available=wait_available
        )
    ...
    if flashes:
        ...
        return flask.render_template(
            "purchase/wait.html", num_tickets=num_tickets, wait_available=wait_available
        )
...
def complete_payment():
    ...
    if flask.request.method == "POST":
        ...
        tickets = (
            ...
            .filter(models.Ticket.owner_id == login.current_user.object_id)
            .filter(models.Ticket.paid == False)  # pylint: disable=singleton-comparison
            .all()
        )
        ...
        if flashes:
            ...
            return flask.render_template(
                "purchase/complete_payment.html", form=flask.request.form
            )

        return payment_logic.do_payment(tickets, postage, method, term, address)
    else:
...
def resell():
    ...
    if cancellation_logic.cancel_tickets(tickets, quiet=True):
        ...
        APP.log_manager.log_event(
            "Cancelled tickets for resale", tickets=tickets, user=login.current_user
        )

        APP.log_manager.log_event(
            "Created tickets from resale", tickets=new_tickets, user=resell_to
        )
...
def buy_postage():
    ...
    if flask.request.method == "POST":
        ...
        if not tickets:
            flashes.append("You have not selected any tickets to buy postage for.")
        ...
        if flashes:
            ...
            return flask.render_template(
                "purchase/buy_postage.html", form=flask.request.form
            )

        return payment_logic.buy_postage(tickets, postage, method, term, address)

    return flask.render_template("purchase/buy_postage.html")


@PURCHASE.route("/purchase/admin_fee/<int:admin_fee_id>", methods=["GET", "POST"])
@login.login_required
    ...
    if not admin_fee:
        ...
        if flask.request.method == "POST":
            ...
            if flashes:
                ...
                return flask.render_template(
                    "purchase/pay_admin_fee.html", fee=admin_fee
                )

            return payment_logic.pay_admin_fee(admin_fee, payment_method, payment_term)
        else:
            return flask.render_template("purchase/pay_admin_fee.html", fee=admin_fee)

    return flask.redirect(
        flask.request.referrer or flask.url_for("dashboard.dashboard_home")
    )
