...
@ADMIN_ANNOUNCEMENTS.route(
    "/admin/announcements/page/<int:page>", methods=["GET", "POST"]
)
...
def announcements(page=1):
    ...
    if flask.request.method == "POST":
        ...
        if "subject" not in form or form["subject"] == "":
            flask.flash("Subject must not be blank", "warning")
            success = False

        if "message" not in form or form["message"] == "":
            flask.flash("Message must not be blank", "warning")
            success = False

        if "owned_tickets" in form and form["owned_tickets"] == "no":
            if "collected" in form and form["collected"] == "yes":
                flask.flash(
                    ("A person cannot own no tickets and have collected " "tickets"),
                    "warning",
                )
                success = False
            if "uncollected" in form and form["uncollected"] == "yes":
                flask.flash(
                    ("A person cannot own no tickets and have uncollected " "tickets"),
                    "warning",
                )
                success = False
        ...
        if success:
            ...
            flask.flash("Announcement created successfully", "success")
...
def delete_announcement(announcement_id):
    ...
    if announcement:
        ...
        flask.flash("Announcement deleted successfully", "success")
    else:
        flask.flash("Could not find announcement, could not delete", "warning")

    return flask.redirect(
        flask.request.referrer or flask.url_for("admin_announcements.announcements")
    )
...
def cancel_announcement_emails(announcement_id):
    ...
    if announcement:
        ...
        flask.flash("Announcement emails cancelled successfully", "success")
    else:
        flask.flash("Could not find announcement, could not cancel emails", "warning")
