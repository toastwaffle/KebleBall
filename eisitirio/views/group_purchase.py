...
def dashboard():
    ...
    if not login.current_user.purchase_group:
        return flask.render_template("group_purchase/dashboard.html")
...
def create():
    ...
    if login.current_user.purchase_group:
        ...
        APP.log_manager.log_event(
            "Created Purchase Group", user=login.current_user, purchase_group=group
        )
...
def join():
    ...
    if login.current_user.purchase_group:
        ...
        group = None
        if "code" in flask.request.form and flask.request.form["code"] != "":
...
def disband():
    ...
    """
    if (
        not login.current_user.purchase_group
        or login.current_user != login.current_user.purchase_group.leader
    ):
        ...
        APP.log_manager.log_event(
            "Disbanded Purchase Group", user=login.current_user, purchase_group=group
        )
...
def leave():
    ...
    if not login.current_user.purchase_group:
        ...
        APP.log_manager.log_event(
            "Left Purchase Group", user=login.current_user, purchase_group=group
        )
...
def update_request():
    ...
    if flask.request.method != "POST":
        return flask.redirect(
            flask.request.referrer or flask.url_for("group_purchase.dashboard")
        )
...
def checkout():
    ...
    if not login.current_user.purchase_group:
        ...
        flask.flash(
            ("The tickets have already been purchased for your purchase " "group."),
            "info",
        )
        return flask.redirect(flask.url_for("dashboard.dashboard_home"))
    ...
    tickets = [
        models.Ticket(
            request.requester, request.ticket_type.slug, request.ticket_type.price
        )
    ...
    flask.flash(
        "Any tickets not paid for within {0} will be cancelled.".format(expiry_time),
        "warning",
    )
