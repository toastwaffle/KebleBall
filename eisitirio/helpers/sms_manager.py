...
class SmsManager(object):
    ...
    def __init__(self):
        self.jinjaenv = jinja2.Environment(
            loader=jinja2.PackageLoader("eisitirio", "templates/sms")
        )
    ...
    def send_template(self, recipient, template, **kwargs):
        ...
        kwargs["template_config"] = {
            key: APP.config[key] for key in APP.config["TEMPLATE_CONFIG_KEYS"]
        }

        return self.send_text(
            recipient, self.jinjaenv.get_template(template).render(**kwargs), sender
        )
    ...
    def send_text(recipient, content, sender):
        ...
        response = requests.post("http://api.txtlocal.com/send/", data=post_data)
        ...
        if response.status_code == requests.codes.ok:
            ...
            if "errors" in result:
                ...
                LOG.debug("Sent SMS to %s (%s)", recipient.phone, recipient.identifier)
        else:
            LOG.error(
                "Failed sending SMS, request returned response %s", response.status_code
            )
