    ...
    return collections.OrderedDict(
        [
            (
                ...
                _maybe_int(
                    DB.session.query(sqlalchemy.func.sum(models.Ticket.price_)).scalar()
                ),
            ),
            (
                "Total value of active tickets",
                _maybe_int(
                    DB.session.query(sqlalchemy.func.sum(models.Ticket.price_))
                    .filter(models.Ticket.cancelled == False)
                    .scalar()
                ),
            ),
            (
            ...
            (
                ...
                _maybe_int(
                    ...
                    .filter(models.EwayTransaction.completed != None)
                    .scalar()
                )
                - _maybe_int(
                    DB.session.query(
            ...
            (
                "Total postage value",
                _maybe_int(
                    DB.session.query(sqlalchemy.func.sum(models.Postage.price))
                    .filter(models.Postage.paid == True)
                    .filter(models.Postage.cancelled == False)
                    .scalar()
                ),
            ),
        ]
...
def _get_payment_methods():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=sqlalchemy_exceptions.SAWarning)
                ...
                (
                    ...
                    models.Ticket.query.join(
                        models.TicketTransactionItem.query.join(
                            ...
                            .filter(models.Transaction.paid == True)
                            .subquery(),
...
def _get_guest_ticket_sales():
    ...
    statistics.update(
        _get_ticket_sales(
            models.Ticket.query.filter(
                models.Ticket.ticket_type.in_(APP.config["GUEST_TYPE_SLUGS"])
            )
        )
    )
...
def _get_ticket_sales(query):
    ...
    return collections.OrderedDict(
        [
            ("Ordered", query.count()),
            ("Cancelled", query.filter(models.Ticket.cancelled == True).count()),
            (
                "Unpaid",
                query.filter(models.Ticket.cancelled == False)
                .filter(models.Ticket.paid == False)
                .count(),
            ),
            (
                "Paid",
                query.filter(models.Ticket.cancelled == False)
                .filter(models.Ticket.paid == True)
                .filter(models.Ticket.collected == False)
                .count(),
            ),
            (
                "Collected",
                query.filter(models.Ticket.cancelled == False)
                .filter(models.Ticket.paid == True)
                .filter(models.Ticket.collected == True)
                .filter(models.Ticket.entered == False)
                .count(),
            ),
            (
                "Entered",
                ...
                .filter(models.Ticket.paid == True)
                .filter(models.Ticket.collected == True)
                .filter(models.Ticket.entered == True)
                .count(),
            ),
        ]
    )
        ...
        [
            (
                ...
                models.User.query.join(
                    models.Waiting.query.subquery(), models.User.waiting
                ).count(),
            ),
            (
                "Tickets Waiting",
                _maybe_int(
                    DB.session.query(
                        sqlalchemy.func.sum(models.Waiting.waiting_for)
                    ).scalar()
                ),
            ),
        ]
    ...
    return collections.OrderedDict(
        [
            ...
            (
                ...
                .filter(models.DietaryRequirements.vegetarian == True)
                .count(),
            ),
            (
                "Pescetarian",
                ...
                .filter(models.DietaryRequirements.vegetarian == False)
                .filter(models.DietaryRequirements.pescetarian == True)
                .count(),
            ),
            (
                "Gluten Free",
