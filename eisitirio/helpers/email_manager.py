...
class EmailManager(object):
    ...
    def smtp_connect(self):
        ...
        try:
            if self.app.config["SMTP_SSL"]:
                self.smtp = smtplib.SMTP_SSL(
                    self.app.config["SMTP_HOST"], self.app.config["SMTP_PORT"]
                )
            else:
            ...
            self.log(
                ...
                "Could not connect to SMTP server at {0}: {1}".format(
                    self.app.config["SMTP_HOST"], error
                ),
            )
            return False
            ...
            try:
                ...
                self.log(
                    ...
                    (
                        ...
                        "login: {1}"
                    ).format(self.app.config["SMTP_HOST"], error),
                )
                return False
        ...
        if self.app.config["SMTP_LOGIN"]:
            try:
                self.smtp.login(
                    self.app.config["SMTP_USER"], self.app.config["SMTP_PASSWORD"]
                )
            except smtplib.SMTPHeloError as error:
                self.log(
                    "error",
    ...
    def get_template(self, template):
        ...
        if self.jinjaenv is None:
            self.jinjaenv = jinja2.Environment(
                loader=jinja2.PackageLoader("eisitirio", "templates/emails")
            )
    ...
    def send_image_html(self, recipient, subject, template, image_bytes, **kwargs):
        ...
        message["Subject"] = "[{0}] {1}".format(self.app.config["BALL_NAME"], subject)
    ...
    def send_template(self, recipient, subject, template, **kwargs):
        ...
        kwargs["template_config"] = {
            key: self.app.config[key] for key in self.app.config["TEMPLATE_CONFIG_KEYS"]
        }

        self.send_text(recipient, subject, template.render(**kwargs), email_from)
    ...
    def send_message(self, message):
        ...
        if not self.app.config["SEND_EMAILS"]:
            self.log("info", "Email not sent per application policy")
            return
        ...
        try:
            self.smtp.sendmail(
                message["From"], message.get_all("To"), message.as_string()
            )
            ...
            self.log(
                ...
                (
                    ...
                    self.app.config["SMTP_HOST"], error.recipients, message["Subject"]
                ),
            )
            ...
            self.log(
                ...
                ).format(self.app.config["SMTP_HOST"], message["Subject"]),
            )
            ...
            self.log(
                ...
                (
                    ...
                    self.app.config["SMTP_HOST"], message["From"], message["Subject"]
                ),
            )
