...
class CustomModelMeta(
    ...
    def get_by(cls, field_name, value):  # this is a metaclass, pylint: disable=E0213
        ...
        if not isinstance(field, sqlalchemy.orm.attributes.InstrumentedAttribute):
            raise AttributeError(
                "Class '{0}' does not have a '{1}' field.".format(
                    cls.__name__, field_name
                )
...
class CustomModel(flask_sqlalchemy.Model):
    ...
    def can(self, name, *args, **kwargs):
        ...
        """
        try:
            return permissions.PERMISSIONS[self.__class__][name](self, *args, **kwargs)
        except KeyError:
            raise AttributeError(
    ...
    def has(self, name, *args, **kwargs):
        ...
        """
        try:
            return permissions.POSSESSIONS[self.__class__][name](self, *args, **kwargs)
        except KeyError:
            raise AttributeError(
