...
USER_ANNOUNCE_LINK = DB.Table(
    ...
    DB.Column("announcement_id", DB.Integer, DB.ForeignKey("announcement.object_id")),
)
...
class Announcement(DB.Model):
    ...
    sender_id = DB.Column(DB.Integer, DB.ForeignKey("user.object_id"), nullable=False)
    sender = DB.relationship(
        "User", backref=DB.backref("announcements-sent", lazy="dynamic")
    )

    college_id = DB.Column(
        DB.Integer, DB.ForeignKey("college.object_id"), nullable=True
    )
    college = DB.relationship(
        "College", backref=DB.backref("announcements", lazy="dynamic")
    )

    affiliation_id = DB.Column(
        DB.Integer, DB.ForeignKey("affiliation.object_id"), nullable=True
    )
    affiliation = DB.relationship(
        "Affiliation", backref=DB.backref("announcements-received", lazy="dynamic")
    )
    ...
    users = DB.relationship(
        "User", secondary=USER_ANNOUNCE_LINK, backref="announcements"
    )

    emails = DB.relationship("User", secondary=EMAIL_ANNOUNCE_LINK, lazy="dynamic")
    ...
    def __init__(
        ...
        if self.college is not None:
            recipient_query = recipient_query.filter(user.User.college == self.college)
        ...
        for recipient in recipient_query.all():
            if (
                (
                    ...
                    or (
                        self.holds_ticket is None
                        or recipient.has_held_ticket() == self.holds_ticket
                    )
                )
                and (self.is_waiting is None or recipient.is_waiting == self.is_waiting)
                and (
                    self.has_collected is None
                    or recipient.has_collected_tickets() == self.has_collected
                )
                and (
                    self.has_uncollected is None
                    or (recipient.has_uncollected_tickets() == self.has_uncollected)
                )
            ):
    ...
    def send_emails(self, count):
        ...
        try:
            for recipient in self.emails:
                ...
                APP.email_manager.send_text(
                    recipient.email, self.subject, self.content, sender
                )
