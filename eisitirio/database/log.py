...
LOG_TICKET_LINK = DB.Table(
    ...
    DB.Column("ticket_id", DB.Integer, DB.ForeignKey("ticket.object_id")),
)
...
class Log(DB.Model):
    ...
    actor_id = DB.Column(DB.Integer(), DB.ForeignKey("user.object_id"), nullable=True)
    actor = DB.relationship(
        "User", backref=DB.backref("actions", lazy="dynamic"), foreign_keys=[actor_id]
    )

    user_id = DB.Column(DB.Integer(), DB.ForeignKey("user.object_id"), nullable=True)
    user = DB.relationship(
        "User", backref=DB.backref("events", lazy="dynamic"), foreign_keys=[user_id]
    )
    ...
    transaction_id = DB.Column(
        DB.Integer(), DB.ForeignKey("transaction.object_id"), nullable=True
    )
    transaction = DB.relationship(
        "Transaction", backref=DB.backref("events", lazy="dynamic")
    )

    purchase_group_id = DB.Column(
        DB.Integer(), DB.ForeignKey("purchase_group.object_id"), nullable=True
    )
    purchase_group = DB.relationship(
        "PurchaseGroup", backref=DB.backref("events", lazy="dynamic")
    )

    admin_fee_id = DB.Column(
        DB.Integer(), DB.ForeignKey("admin_fee.object_id"), nullable=True
    )
    admin_fee = DB.relationship(
        "AdminFee", backref=DB.backref("events", lazy="dynamic")
    )
    ...
    def write_csv_row(self, csv_writer):
        ...
        csv_writer.writerow(
            [
                ...
                (self.transaction_id if self.transaction_id is not None else "N/A"),
                (
