...
class PostageTransactionItem(transaction_item.TransactionItem):
    ...
    object_id = DB.Column(
        DB.Integer(), DB.ForeignKey("transaction_item.object_id"), primary_key=True
    )

    is_refund = DB.Column(DB.Boolean, nullable=False, default=False)

    postage_id = DB.Column(
        DB.Integer, DB.ForeignKey("postage.object_id"), nullable=False
    )
    postage = DB.relationship(
        "Postage", backref=DB.backref("transaction_items", lazy="dynamic")
    )
    ...
    def description(self):
        ...
        return "{0}{1} Postage".format(
            "Refund of " if self.is_refund else "", self.postage.postage_type
        )
