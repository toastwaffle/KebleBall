...
POSTAGE_TICKET_LINK = DB.Table(
    "postage_ticket_link",
    DB.Model.metadata,
    DB.Column("postage_id", DB.Integer, DB.ForeignKey("postage.object_id")),
...
class Postage(DB.Model):
    ...
    tickets = DB.relationship(
        ...
        backref=DB.backref("postage", lazy=False, uselist=False),
        lazy="dynamic",
    )

    owner_id = DB.Column(DB.Integer, DB.ForeignKey("user.object_id"), nullable=True)
    owner = DB.relationship(
        "User", backref=DB.backref("postage_entries", lazy="dynamic")
    )
