...
class GroupPurchaseRequest(DB.Model):
    ...
    ticket_type_slug = DB.Column(DB.Unicode(50), nullable=False)
    number_requested = DB.Column(DB.Integer(), nullable=False)

    purchase_group_id = DB.Column(
        DB.Integer, DB.ForeignKey("purchase_group.object_id"), nullable=False
    )
    ...
    def __init__(self, ticket_type_slug, number_requested, purchase_group, requester):
    ...
    def __repr__(self):
        return "<GroupPurchaseRequest({0}): {1} {2} tickets>".format(
            self.object_id, self.number_requested, self.ticket_type_slug
        )
